...
bool display_marker(float x, float y, int marker_action, int marker_id){
...
int main( int argc, char** argv )
{
  ...
  display_marker(goal_pu1_x, goal_pu1_y, add_marker, marker_no);
  
  // Publish marker at pickup zone 2
  marker_no = 2;
  display_marker(goal_pu2_x, goal_pu2_y, add_marker, marker_no);
  ...
  while (ros::ok()){
    ...
    if (abs(goal_pu1_x-amcl_pos_x) < 0.13 && abs(goal_pu1_y-amcl_pos_y) < 0.13){
      ...
      display_marker(goal_pu1_x, goal_pu1_y, add_marker, marker_no); // Delete marker #1
    }
    // @pickup zone 2:
      ...
      display_marker(goal_pu2_x, goal_pu2_y, add_marker, marker_no); // Delete marker
    }
    // @dropoff zone:
      ...
      add_marker = 1; // ADD marker
      marker_no = 3;
      display_marker(goal_do1_x, goal_do1_y, add_marker, marker_no); // Publish marker
    }
